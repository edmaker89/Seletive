...
def nova_vaga(request):
    if request.method == "POST":
        ...
        return redirect (f'/empresa/empresa/{empresa}')
...
def nova_tarefa(request, id_vaga):
    ...
    try:
        ...
        messages.add_message(request, constants.ERROR, 'Erro ao criar tarefa.')
...
def realizar_tarefa(request, id):
    ...
    if not tarefa_list.exists():
        messages.add_message(request, constants.ERROR, 'Tarefa não encontrada.')
        redirect ('/empresa/empresas')
