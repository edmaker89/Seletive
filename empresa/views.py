...
def nova_empresa(request):
    if request.method == "GET":
        ...
        if (len(nome.strip()) == 0 or len(email.strip()) == 0 or len(cidade.strip()) == 0 or len(endereco.strip()) == 0 or len(nicho.strip()) == 0 or len(caracteristicas.strip()) == 0 or (not logo)): 
            ...
            return redirect('/empresa/nova_empresa')

        if logo.size > 100_000_000:
            messages.add_message(request, constants.ERROR, 'A logo da empresa deve ter menos de 10MB')
            return redirect('/empresa/nova_empresa')

        if nicho not in [i[0] for i in Empresa.choices_nichos_mercado]:
...
def empresas(request):
    ...
    empresas = Empresa.objects.all() #listar todas as empresas antes do filtro
    print(tecnologias_filtrar)
    print(nome_filtrar)
    
    if tecnologias_filtrar:
        empresas = empresas.filter(tecnologia__id=tecnologias_filtrar)
